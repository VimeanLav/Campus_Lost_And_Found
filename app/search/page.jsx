...
export default function SearchPage() {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    setLoading(true);
    let url = `/api/report/found?search=${encodeURIComponent(search)}&category=${encodeURIComponent(category)}`;
    try {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {items.length === 0 ? (
            ...
            items.map(item => (
              <div key={item._id} className="block text-center border rounded-lg p-4 shadow cursor-pointer" onClick={() => handleItemClick(item._id)}>
